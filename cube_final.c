...
int main() {
  ...
  init_cube(&cube1);
  normal_cube(&cube1);
  printf("Give the moves to randomize the cube:\n");
  struct move f;
  int type;
  while (1) {
    printf("Write the face - \n(0 - front face, 2 - right face, 4 - top face, -1 - end) \n\e[1mENTER ONLY THESE FACES\e[m\n");
    scanf("%d", &f.face);
    if (f.face == -1) {
      break;
    }
    printf("Write the move - \n(0 - clockwise, 1 - clockwise) \n\e[1mENTER ONLY THESE MOVES\e[m\n");
    scanf("%d", &type);
    if (type == 0) {
      rotate_cube_clockwise(&cube1, f);
    }
    if (type == 1) {
      rotate_cube_anti_clockwise(&cube1, f);
    }
    printf("The current state of the cube is:\n");
    print_cube(cube1);
  }
